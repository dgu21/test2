...
function privateInfo_new(room_num, user_from, user_to, message, speaker) {
    this.room_num = room_num;
    this.user_from = user_from;
    this.user_to = user_to;
    this.message = message;
...
socket.on('receive request', function (msg) {
    if (msg.request_to == current_username) {
        ...
        var temp = msg.request_from + ' ' + 'wants to add you as friend!';
        $('#request-from-who').append(temp);
...
socket.on('try create room', function (msg) {
    if (current_username == msg.user_to) {
        ...
        var temp = msg.user_from + ' ' + 'wants to create room with you!';
        $('#room-request-from-who').append(temp);
...
socket.on('update-private-room', function (msg) {
    if ((current_username == msg.user_to) || (current_username == msg.user_from)) {
        ...
        var div_append_content = '<div class="div" id=\'' + div_id + '\'></div>';
        $('#private-room-list').append(div_append_content);

        // new list id
        var list_id = 'list-' + num; // new list id
        var list_append_content = '<div class="private-area" id=\'' + list_id + '\'></div>'; /////////
        var list_append_content_query = '#' + list_id;
        var div_query = '#' + div_id; //#

        var d = 'header-' + num;
        var a = '';
        a += '<button class="ui toggle button" id=\'';
        a += d;
        ...
        // a += d;
        // a += '\' class="ui header">';
        // a += '<i class="tag icon"></i>';
        // a += '<div class="content">';
        // a += 'Room ' + msg.room_num;
        // a += '</div>';
        //
        // a += '<button class="circular ui icon button" onclick="showContent(\'';
        // a += num;
        // a += '\'';
        // a += ');">';
        // a += '<i class="icon setting"></i>';
        // a += '</button>';
        // a += '</h3>';
        ...
        $(list_append_content_query).hide();
        ...
        b += '<div id=\'';
        b += c;
        b += '\' class="ui action input">';
        b += '<input id=';
        b +=  input_id;
        b +=  ' placeholder="message to send..." type="text">';
        b += '<button class="ui button" id=\'';
        b += button_id;
        b += '\' onclick="pubSend(\'';
        b += temp_room_num;
        b += '\',\'';
        ...
        b += 'Send';
        b += '</button>';
        // b += '<button class="ui button">Send</button>';
        b += '</div>';
...
function showContent(num) {
    var list_query_to_hide = '#' + 'list-' + show;
    var content_query_to_hide = '#' + 'content-' + show;
    var list_query_to_show = '#' + 'list-' + num;
    var content_query_to_show = '#' + 'content-' + num;

    show = num;

    $(list_query_to_hide).hide();
    $(content_query_to_hide).hide();
    $(list_query_to_show).show();
    $(content_query_to_show).show();

    // document.getElementById(list_query_to_hide).setAttribute('hidden');
    // document.getElementById(content_query_to_hide).setAttribute('hidden');
    // document.getElementById(list_query__to_show).removeAttribute('hidden');
    // document.getElementById(content_query_to_show).removeAttribute('hidden');
}
...
function pubSend(room_num, user_from, user_to) {
    ...
    var info = new privateInfo_new(room_num, user_from, user_to, input, current_username);
...
socket.on('update private room', function (msg) {
    if ((current_username == msg.user_to) || (current_username == msg.user_from)) {
        ...
        // console.log('user to:' + msg.user_to);
        // console.log('user from:' + msg.user_from);

        var temp = '';
        temp += '<div class="item" id="public-message">';
        // temp += '<img class="ui avatar image" src="image/private.png">';
        temp += '<div class="content">';
        temp += '<a class="header">';
